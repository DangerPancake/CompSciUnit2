...
public class BinaryTree
{
   ...
   private TreeNode add(TreeNode tree, Comparable item)
   {
    ...
    private void inOrderTraversal(TreeNode tree, List<Object> list) {
      if(tree != null) {
         inOrderTraversal(tree.getLeft(), list);  // Visit left subtree
         list.add(tree.getValue());                // Process current node
         inOrderTraversal(tree.getRight(), list); // Visit right subtree
      }
    }
    ...
    private void postOrderTraversal(TreeNode tree, List<Object> list) {
      if(tree != null) {
         postOrderTraversal(tree.getLeft(), list);  // Visit left subtree
         postOrderTraversal(tree.getRight(), list); // Visit right subtree
         list.add(tree.getValue());                  // Process current node
      }
    }
